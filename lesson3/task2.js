...
class Bank extends EventEmmiter {
    ...
    _send(senderId, recepientId, sum) {
        this._validateTransaction(senderId, sum, "withdrow");
        this._validateTransaction(recepientId, sum, "add");
        this.contragents = this.contragents.map(contragent => {
            if (contragent.personId === senderId) {
                return { ...contragent, balance: contragent.balance - sum }
            }
            if (contragent.personId === recepientId) {
                return { ...contragent, balance: contragent.balance + sum }
            }
            return contragent
        })
        console.log(this.contragents);
    }
...
bank.on("send", function (senderId, recepientId, sum) {
    this._send(senderId, recepientId, sum)
})
